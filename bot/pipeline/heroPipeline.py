...
sourceNotes = {
    "3d": "the judge approved the 3D render",
    "2d": "the judge rejected the 3D render, so the ad uses the photo cutout",
    "3d-unapproved": "the judge rejected the 3D render and there is no clean cutout, so the ad uses the last render",
}
...
class HeroPipeline:
    ...
    async def revise(self, job, action, feedback, progress):
        ...
        if action is FeedbackAction.REEDIT:
            ...
            await progress(f"Re-rendering the current model with {describeParams(params)}.")
    ...
    async def regenerate(self, job, progress, params=None):
        ...
        s.update(image=str(image), cutout=str(cutout) if cutout else None)
    ...
    async def rerender(self, job, progress, params=None, keepCopy=False, regenerateIfMissing=True):
        ...
        def work(client, say):
            ...
            return runHero(client, s["objName"], s["image"], s["cutout"], blurb, outDir=outDir,
                           ...
                           engine=self.settings.engine, params=params, onEvent=say)
        ...
        copy, heroes = trace["copy"], trace["heroes"]
        if keepCopy and s.get("copy"):  # a camera change shouldn't reword an ad the reviewer already read
            copy = s["copy"]
            heroes = await asyncio.to_thread(composeHeroes, trace["product"], copy, outDir)
        s.update(copy=copy, params=trace["attempts"][-1]["params"], product=trace["product"], source=trace["source"])
        job.outputs = list(heroes.values())
        last = trace["attempts"][-1]["verdict"]
        await progress(f"Hero shots ready after {len(trace['attempts'])} render attempt(s): "
                       f"{sourceNotes.get(trace['source'], trace['source'])}. Judge: {last['reason']}")
    ...
    async def reedit(self, job, feedback, progress):
        ...
        await progress(f"New copy: {describeCopy(copy)}{dropped}")
...
def makeModel(client, say, source, name, previous, timeout):
    ...
    def onProgress(update):
        text = (f"Hyper3D job submitted (balance {update.get('balance')})." if isinstance(update, dict)
                else f"Hyper3D: {', '.join(update)}")
        if last[-1:] != [text]:
            last.append(text)
            say(text)
...
def describeParams(p):
    return (f"azimuth {p['azimuth']}°, elevation {p['elevation']}°, rotation {p['rotationZ']}°, "
            f"{p['lighting']} lighting, padding {p['padding']}")


def describeCopy(c):
    return (f"**{c.title}**" + (f" — {c.tagline}" if c.tagline else "") + "".join(f"\n• {s}" for s in c.specs)
