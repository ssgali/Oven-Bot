...
from compose.videoOverlay import compositeOverlays, framePattern

previewFrameName = "preview_{shotId}.png"


def _renderRange(client, jobPaths, script, startFrame, endFrame):
    ...
    compositeOverlays(jobPaths.rawDir, script, jobPaths.compositedDir, startFrame, endFrame)
    return encodeFrames(jobPaths.compositedDir, jobPaths.videoPath, script["fps"])


def makeDirectorTools(client, jobPaths, scriptBox):
    ...
    to a new script, so every tool below always acts on the currently-proposed script."""
    ...
    def render_range(startFrame: int, endFrame: int) -> dict:
        ...
        return _renderRange(client, jobPaths, scriptBox["script"], startFrame, endFrame)
    ...
    def extract_preview_frame(shotId: str) -> str:
        ...
        if not composited.is_file():
            compositeOverlays(jobPaths.rawDir, script, jobPaths.compositedDir, frame, frame)
