...
Live run of the Discord bot: a seller message becomes an ad job, the image→GLB endpoint builds the 3D model, and the hero shots come back in-thread. GitHub doesn't autoplay committed video inside the rendered README, so this link opens GitHub's own player — full quality, with audio.
